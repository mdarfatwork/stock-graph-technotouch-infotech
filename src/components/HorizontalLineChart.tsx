...
const HorizontalLineChart: React.FC<HorizontalLineChartProps> = ({
  ...
  const last10High = Math.max(...last10Candles.map((candle) => candle.high));
  const last10Low = Math.min(...last10Candles.map((candle) => candle.low));

  const last10StartIndex = Math.max(0, data.length - 10);

  const highLineData = data.map((_, index) => {
    if (index >= last10StartIndex) {
      return {
        x: index,
        y: last10High,
      };
  ...
  const lowLineData = data.map((_, index) => {
    if (index >= last10StartIndex) {
      return {
        x: index,
        y: last10Low,
      ...
      labels: {
        ...
        formatter: (value) => {
          if (value === null || value === undefined || isNaN(value)) {
            console.log("Y-axis formatter received invalid value:", value);
            return "";
          }
  ...
  const areaHeight = (last10High - last10Low) * 0.2;
  const areaTopLevel = last10Low + areaHeight;

  const rangeAreaData = data.map((_, index) => {
    if (index >= last10StartIndex) {
      return {
        x: index,
        y: [last10Low, areaTopLevel],
      };
    }
    return {
      x: index,
